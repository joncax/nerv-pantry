...
import { Package, Search, Filter, Plus, Clock, Trash2, CheckCircle, ScanLine, X } from 'lucide-react'
...
export default function Inventory() {
  ...
  // Pesquisa por texto aplicada sobre os resultados já filtrados
    ...
    <div className="space-y-4">
      ...
      <div className="flex gap-2">
        <div className="flex-1 flex items-center gap-2 px-3 py-2 rounded border"
          ...
          {search && (
            <button onClick={() => setSearch('')} style={{ display: 'flex', color: 'var(--color-nerv-muted)' }}>
              <X size={13} />
            </button>
          )}
        </div>
      ...
      {chips.length > 0 && (
        <div className="flex items-center gap-2 flex-wrap">
          {chips.map(chip => (
            <span key={chip.key} className="flex items-center gap-1 px-2 py-1 rounded text-xs"
              style={{ background: 'var(--color-nerv-border)', color: 'var(--color-nerv-text)' }}>
              {chip.label}
              <button onClick={chip.onRemove} style={{ display: 'flex', color: 'var(--color-nerv-muted)' }}>
                <X size={11} />
              </button>
            </span>
          ))}
          {chips.length > 1 && (
            <button onClick={clearAllFilters} className="text-xs"
              style={{ color: 'var(--color-nerv-accent)' }}>
              Limpar tudo
            </button>
          )}
        </div>
      ...
      <div className="rounded-lg border overflow-hidden"
        ...
        {isLoading ? (
          ...
          <div className="px-4 py-12 text-center">
            ...
            {hasAnyFilter ? (
              <button onClick={() => { clearAllFilters(); setSearch('') }} className="text-xs mt-1"
                style={{ color: 'var(--color-nerv-accent)' }}>
                Limpar filtros
              </button>
            ) : (
          ...
          displayItems.map(item => <InventoryRow key={item.id} item={item} onConsume={setConsumeItem} />)
        )}
      </div>
    </div>
  )
