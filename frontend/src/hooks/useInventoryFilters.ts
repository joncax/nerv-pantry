...
export function useInventoryFilters(items: InventoryItem[]) {
  ...
  const filterOptions: FilterOptions = useMemo(() => {
    ...
    for (const item of items) {
      if (item.location) {
        ...
        else locationMap.set(item.location_id, { name: item.location.name, count: 1 })
      }
      if (item.product?.category_id != null && item.product.category) {
        const catId = item.product.category_id
        const entry = categoryMap.get(catId)
        if (entry) entry.count++
        else categoryMap.set(catId, { name: item.product.category.name, count: 1 })
      }
    }
  ...
  const filteredItems = useMemo(() => {
    ...
    if (filters.categories.length > 0) {
      result = result.filter(
        item => item.product?.category_id != null &&
    ...
    if (filters.favoritesOnly) {
      result = result.filter(item => item.product_is_favorite)
    }
  ...
  const activeFilterCount = useMemo(() => {
    ...
    if (filters.favoritesOnly)            count++ // U5-C
    return count
  }, [filters])
